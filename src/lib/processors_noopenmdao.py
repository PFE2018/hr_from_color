...
class findFaceGetPulse(object):
    ...
    def run(self, cam):
        ...
        #modifier ici pour rectangle dynamique
        detected = list(self.face_cascade.detectMultiScale(self.gray,
                                                           scaleFactor=1.3,
                                                           minNeighbors=4,
                                                           minSize=(
        ...
        if L > 10:
            ...
            #since the video have about 4x faster framerate we divise by a factor
            #to adjust the framerate to that of a webcam
            #Scale to a framerate of 16 fps
            divisionFactor = self.fps/16
            self.freqs = float(self.fps) / (divisionFactor * L) * np.arange(L / 2 + 1)


            #frequence a Battements Par Minute
            freqs = 60. * self.freqs
            idx = np.where((freqs > 50) & (freqs < 180))
